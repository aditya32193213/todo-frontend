...
import {
  ...
  AlertTriangle,                       // new icon for metrics warning
} from "lucide-react";
...
import { STATUSES } from "../features/tasks/taskConstants";
...
/** Returns a responsive skeleton count based on viewport width */
...
const Home = () => {
  ...
  const {
    ...
    metrics, metricsError,                                                    // ← metricsError
        ...
        <main className="flex-1 min-w-0 overflow-y-auto">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-7 space-y-6">
            ...
            <div className="relative">
              {metricsError && (
                <div className="text-xs flex items-center gap-1 text-amber-500 mb-2">
                  <AlertTriangle size={12} />
                  Could not load latest stats
                </div>
              )}
            ...
            <div className="space-y-3 animate-slide-up">
              ...
              <div className="flex flex-col sm:flex-row gap-2.5">
                ...
                <div ref={sortRef} className="relative shrink-0">
                  ...
                  {sortOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 z-30 rounded-xl overflow-hidden bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-xl animate-slide-up">
                      <div className="p-1">
                        {SORT_OPTIONS.map((opt) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-display font-medium transition-all duration-150 ${isSelected ? "bg-violet-500/10 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300" : "text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-800 dark:hover:text-white"}`}
                            >
                              <span>{opt.label}</span>
                              {isSelected && (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <span className="text-xs font-display font-semibold text-slate-400 dark:text-slate-500">Filter:</span>
                </div>
                ...
                {STATUSES.map((s) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-display font-semibold transition-all duration-200 ${statusFilter === s.value ? "bg-gradient-to-r from-violet-500 to-indigo-500 text-white shadow-md shadow-violet-500/25" : "bg-white/60 dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 border border-slate-200/80 dark:border-white/8 hover:bg-white dark:hover:bg-white/8 hover:text-slate-700 dark:hover:text-slate-200 backdrop-blur-sm"}`}>
                    {s.label}
                  </button>
                ))}
                ...
                {activeFilters > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-display font-semibold text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-200/60 dark:border-red-500/20 hover:bg-red-100 dark:hover:bg-red-500/20 transition-all duration-200">
                    <X size={11} />
            ...
            <div className="space-y-2.5">
              ...
              {/* Loading: responsive skeleton count */}
              {loading ? (
                Array.from({ length: skeletonCount }).map((_, i) => <SkeletonCard key={i} />)
              ) : error ? (
                <div className="rounded-2xl border border-red-200/60 dark:border-red-500/20 bg-red-50/40 dark:bg-red-500/[0.04] backdrop-blur-sm p-10 flex flex-col items-center gap-4 text-center animate-fade-in">
                  <div className="w-14 h-14 rounded-2xl bg-red-100 dark:bg-red-500/15 flex items-center justify-center">
                    <AlertCircle size={24} className="text-red-500" />
                  </div>
                  <div>
                    <h3 className="text-base font-display font-bold text-slate-700 dark:text-slate-200">Failed to load tasks</h3>
                    <p className="text-sm font-body text-slate-400 dark:text-slate-500 mt-1">{error}</p>
                  </div>
                  <button
                    onClick={fetchTodos}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-display font-semibold text-white bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 shadow-md shadow-violet-500/20 transition-all duration-200">
                    Try again
                  </button>
                </div>
              ) : todos.length === 0 ? (
                <div className="rounded-2xl border border-slate-200/60 dark:border-white/8 bg-white/40 dark:bg-white/[0.02] backdrop-blur-sm p-14 flex flex-col items-center gap-5 text-center animate-fade-in">
                  <div className="w-16 h-16 rounded-2xl animate-float bg-gradient-to-br from-violet-100 to-indigo-100 dark:from-violet-500/15 dark:to-indigo-500/15 border border-violet-200/50 dark:border-violet-500/20 flex items-center justify-center">
                    {isFiltering ? <Search size={26} className="text-violet-400" /> : <Sparkles size={26} className="text-violet-400" />}
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap justify-center">
                    {isFiltering ? (
                      <button onClick={clearAllFilters} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-display font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/8 border border-slate-200/80 dark:border-white/8 hover:bg-slate-200 dark:hover:bg-white/12 transition-all duration-200">
                        <X size={14} /> Clear filters
                      </button>
                    ) : (
                      <>
                        <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-display font-semibold text-white bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 shadow-md shadow-violet-500/20 transition-all duration-200">
                          <Plus size={14} /> Add Task
                        </button>
                        <button onClick={handleSeed} disabled={seeding} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm
             ...
             disabled:opacity-50 transition-all duration-200">
                          {seeding ? <><Loader2 size={14} className="animate-spin" /> Seeding…</> : <><ClipboardList size={14} /> Seed sample tasks</>}
                        </button>
                      </>
                    )}
                  </div>
            ...
            {!loading && !error && todos.length > 0 && totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pb-4 animate-fade-in">
                <button onClick={() => setPage((p) => p - 1)} disabled={page === 1}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-display font-semibold text-slate-500 dark:text-slate-400 bg-white/60 dark:bg-white/[0.04] border border-slate-200/80 dark:border-white/8 hover:bg-white dark:hover:bg-white/8 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 backdrop-blur-sm">
                  <ChevronLeft size={14} />
                ...
                <div className="flex items-center gap-1">
                  {pageNumbers.map((p) => (
                    <button key={p} onClick={() => setPage(p)}
                      className={`w-9 h-9 rounded-xl text-sm font-display font-semibold transition-all duration-200 ${p === page ? "bg-gradient-to-br from-violet-500 to-indigo-500 text-white shadow-md shadow-violet-500/25" : "text-slate-500 dark:text-slate-400 bg-white/60 dark:bg-white/[0.04] border border-slate-200/80 dark:border-white/8 hover:bg-white dark:hover:bg-white/8 backdrop-blur-sm"}`}>
                      {p}
                    </button>
                  ))}
                </div>
